...
function VendasDados({
  ...
  async function getVendasData() {
    ...
    await (async function processDespesasData() {
      try {
        ...
        const { data: getContasPagas } = await Axios.get(
          "http://localhost:5000/pagasTabela",
          {
            params: {
        ...
        const totalPagas = getContasPagas.reduce((a, b) => ({
    ...
    await (async function processDevoData() {
      try {
        const { data: getDevolucoes } = await Axios.get(
          "http://localhost:5000/devolucoes",
          {
            params: {
              currentBalanceDate: currentBalanceDate,
              prevBalanceDate: prevBalanceDate,
        ...
        if (getDevolucoes.length > 0) {
          devoTotal = getDevolucoes.reduce((a, b) => ({
            total: a.total + b.total,
          }));
          devoB2W = getDevolucoes.filter(({ descricao }) =>
            descricao.includes("B2W")
          );
        } else {
          devoTotal["total"] = 0;
          devoB2W = 0;
        ...
        devoB2W.length > 0
          ? (dreData["devoB2W"] = devoB2W[0].total)
          : (dreData["devoB2W"] = 0);
    ...
    await (async function processVendasData() {
      try {
        ...
        dreData["vendasBruta"] =
          getTotalVendas[0].totalvendas - dreData.devoTotal;
  ...
  return (
    <>
      ...
      <div className="col-lg-2" data-testid="isLive">
