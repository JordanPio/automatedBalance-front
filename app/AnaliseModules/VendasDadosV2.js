...
function VendasDados({
  ...
  const pieChartLucro = {
    ...
    datasets: [
      {
        data: [
          balanceData.lucroLojaFisica,
          balanceData.lucroB2W,
          balanceData.lucroMagazineLuiza,
          balanceData.lucroMercadoPago,
        ],
  ...
  async function getVendasData() {
    ...
    await (async function processDespesasData() {
      try {
        const { data: getContasPagas } = await Axios.get(
          "http://localhost:5000/pagasTest",
          {
            params: {
        ...
        function getTotalFiltered(dataset, accountType, accountDescription) {
          ...
          if (dataset.filter(findAccount).length > 0) {
            return dataset
              .filter(findAccount)
              .reduce((a, b) => ({ total: a.total + b.total })).total;
          } else {
            return 0;
          }
        }
        ...
        dreData["taxasB2W"] = getTotalFiltered(
          getContasPagas,
          "custom",
          "Taxas B2W"
        );
        dreData["freteB2W"] = getTotalFiltered(
          getContasPagas,
          "custom",
          "Frete B2W"
        );
        dreData["devoB2W"] = getTotalFiltered(
          getContasPagas,
          "custom",
          "Devolucao B2W"
        );
        dreData["taxasMagazineLuiza"] = getTotalFiltered(
          getContasPagas,
          "custom",
          "Taxas Magazine"
        );
        dreData["taxasMercadoPago"] = getTotalFiltered(
          getContasPagas,
          "custom",
          "Taxas Mercado"
        );
        dreData["imposto"] = getTotalFiltered(
          getContasPagas,
          "conta",
          "Impostos"
        );
        ...
        function getDevolucoes() {
          return getContasPagas
            .filter(
              ({ conta }) => conta.includes("Devo") || conta.includes("Dif")
            )
            .reduce((a, b) => ({ total: a.total + b.total })).total > 0
            ? getContasPagas
                .filter(
                  ({ conta }) => conta.includes("Devo") || conta.includes("Dif")
                )
        ...
        dreData["totalPagas"] =
          getContasPagas.reduce((a, b) => ({ total: a.total + b.total }))
            .total -
          dreData.devoTotal -
          dreData.devoB2W -
          dreData.taxasB2W -
          dreData.freteB2W -
          dreData.taxasMagazineLuiza -
          dreData.taxasMercadoPago -
          dreData.imposto;
        ...
        console.error(
          "Error processing Despesas Data in VendasDados Component"
        );
      }
    })();
    ...
    await (async function processVendasData() {
      try {
        const { data: getVendasOnline } = await Axios.get(
          "http://localhost:5000/vendasOnline",
          {
            params: {
              currentBalanceDate: currentBalanceDate,
              prevBalanceDate: prevBalanceDate,
        ...
        if (getVendasOnline.length <= 0) {
          ...
          totalVendasOnline = getVendasOnline.reduce((a, b) => ({
            totalvendas: a.totalvendas + b.totalvendas,
          }));
        }
        Object.keys(totalVendasOnline).length > 0
          ? (dreData["vendasOnline"] = totalVendasOnline.totalvendas)
          : (dreData["vendasOnline"] = 0);

        const { data: getTotalVendas } = await Axios.get(
          "http://localhost:5000/totalVendas",
          {
            params: {
              currentBalanceDate: currentBalanceDate,
              prevBalanceDate: prevBalanceDate,
        ...
        dreData["vendasBruta"] =
          getTotalVendas[0].totalvendas - dreData.devoTotal - dreData.devoB2W;
        dreData["vendasLojaFisica"] =
          getTotalVendas[0].totalvendas - dreData.vendasOnline;
        dreData["percentFisica"] = (
          (dreData["vendasLojaFisica"] / dreData["vendastotal"]) *
          100
        ).toFixed(2);
        dreData["percentOnline"] = (
          (dreData.vendasOnline / dreData["vendastotal"]) *
          100
        ).toFixed(2);
        ...
        const { data: getVendasDRE } = await Axios.get(
          "http://localhost:5000/vendasdre",
          {
            params: {
              currentBalanceDate: currentBalanceDate,
              prevBalanceDate: prevBalanceDate,
        ...
        if (getTotalVendas[0].totalvendas === getVendasDRE[0].totalvendido)
          console.log(
            "Valores total de Vendas Batem corretamente!!!",
            "Query Vendas Periodo = ",
            getTotalVendas[0].totalvendas,
            "Query Vendas Totais = ",
            getVendasDRE[0].totalvendido
          );
        else if (
          getTotalVendas[0].totalvendas / getVendasDRE[0].totalvendido >=
            0.98 &&
          getTotalVendas[0].totalvendas / getVendasDRE[0].totalvendido < 1.02
        )
          console.log(
            "Valores total de Vendas batem mais estao com uma discrepancia de ate 2% em vendasDados",
            "Query Vendas Periodo = ",
            getTotalVendas[0].totalvendas,
            "Query Vendas Totais = ",
            getVendasDRE[0].totalvendido
          ...
          console.log(
            "Valores total de Vendas nao batem em vendasDados",
            "Query Vendas Periodo = ",
            getTotalVendas[0].totalvendas,
            "Query Vendas Totais = ",
            getVendasDRE[0].totalvendido
    ...
    (
      await function createDadosSection() {
        ...
        dreData["lucroSobFaturamento"] = (
          (dreData.lucroLiquido / dreData.vendasBruta) *
          100
        ).toFixed(2);
        ...
        dreData["lucroB2W"] =
          Math.round(
            ((dreData.B2W - dreData.freteB2W) * 0.311583 -
              dreData.taxasB2W -
              dreData.B2W * 0.05) *
              100
          ) / 100;
        dreData["lucroMagazineLuiza"] =
          Math.round(
            (dreData.magazineLuiza * 0.311583 -
              dreData.taxasMagazineLuiza -
              dreData.magazineLuiza * 0.05) *
              100
          ) / 100;
        dreData["lucroMercadoPago"] =
          Math.round(
            (dreData.mercadoPago * 0.311583 -
              dreData.taxasMercadoPago -
              dreData.mercadoPago * 0.05) *
              100
          ) / 100;
        dreData["lucroLojaFisica"] =
          Math.round(
            (dreData.lucroBruto -
              (dreData.lucroB2W +
                dreData.lucroMagazineLuiza +
        ...
        dreData["lucroSobFaturamentoB2W"] = (
          (dreData.lucroB2W / (dreData.B2W - dreData.freteB2W)) *
          100
        ).toFixed(2);
        dreData["lucroSobFaturamentoMagazine"] = (
          (dreData.lucroMagazineLuiza / dreData.magazineLuiza) *
          100
        ).toFixed(2);
        dreData["lucroSobFaturamentoFisica"] = (
          (dreData.lucroLojaFisica / dreData.vendasLojaFisica) *
          100
        ).toFixed(2);
      }
    )();
    ...
    await (async function processEstoqueData() {
      try {
        const { data: getCurrentEstoque } = await Axios.get(
          "http://localhost:5000/totalEstoque",
          {
            params: {
              currentBalanceDate: currentBalanceDate,
              newBalanceDate: newBalanceDate,
        ...
        if (getCurrentEstoque.length > 0)
          dreData["estoque"] = getCurrentEstoque[0].custototal;
    ...
    if (newBalanceDate.length > 0)
      updateDbEstoque(dreData.estoque, newBalanceDate);

    setBalanceData(dreData);
  }

  async function updateDbEstoque(totalEstoque, currentDate) {
    const estoqueData = await Axios.post(
      ...
      .then((resp) => {
        if (resp) {
          console.log(
            "Sucessfull updated Estoque into DB when creating new balance"
          );
        }
        setNewBalDataUpdate();
      })
  ...
  return (
    <>
      ...
      <div className="col-lg-2">
        ...
        <h6 className="mt-4">
          Vendas Total: R${formatNumber(balanceData.vendastotal)}
        </h6>
        <h6 className="mt-4">
          Loja Fisica: R${formatNumber(balanceData.vendasLojaFisica)}
        </h6>
        <h6 className="mt-4">
          Loja Online: R${formatNumber(balanceData.vendasOnline)}
        </h6>
        <h6 className="mt-4">
          Estoque Atual: R${formatNumber(balanceData.estoque)}
        </h6>
        <h6 className="mt-4">
          Lucro Liquido Sob Faturamento: {balanceData.lucroSobFaturamento}%
        </h6>
        <h6 className="mt-4">
          Lucro Bruto Sob Faturamento B2W: {balanceData.lucroSobFaturamentoB2W}%
        </h6>
        <h6 className="mt-4">
          Lucro Bruto Sob Faturamento Magazine:{" "}
          {balanceData.lucroSobFaturamentoMagazine}%
        </h6>
        <h6 className="mt-4">
          Lucro Bruto Sob Faturamento Fisica:{" "}
          {balanceData.lucroSobFaturamentoFisica}%
        </h6>
      </div>
        ...
        <table className="table table-striped table-sm table-hover mt-4">
          ...
          <tbody>
            ...
            <tr>
              <td>(-) Devolucoes</td>
              <td>
                R${formatNumber(balanceData.devoTotal + balanceData.devoB2W)}
              </td>
            </tr>
            <tr>
              <td>
